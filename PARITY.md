...
| Sessions and history | done | New/switch/delete/rename, pins, search, cwd-aware resume, draft-preserving same-session process recovery, serialized deletion that refuses attached/busy panes, safe header validation, and history attachment reconstruction. | Export and branch-from-message remain `next`. |
| Concurrent panes | partial | Two independently focused panes, streaming-safe normal-click routing, drag/split, pop-outs, shared-client event fanout, pane-scoped drafts, and safe independent project rebinding. | Persisted arbitrary split trees and more than two panes remain `next`. |
