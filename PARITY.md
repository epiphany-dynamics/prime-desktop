...
| Agents and messaging | partial | Agents dashboard reads Prime `session-artifacts/**/rlm-subagents.jsonl` plus live daemon `children` / `rlm_child_update`, lists every sub-agent for the focused chat, and opens transcripts (click) or the live session (⌘/Ctrl-click). Inline subagent summaries remain. | Full nested worker admin (stop/cancel graph, message-all) remains `next`. |
