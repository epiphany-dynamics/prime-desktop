...
function rpcState(state, streamingMessage) {
  ...
  return {
    ...
    isRunningTools: !!(state && state.isRunningTools),
    hasRunningRlmChildren: !!(state && state.hasRunningRlmChildren),
...
class DaemonRpcAdapter extends EventEmitter {
  constructor({ socketPath, sessionPath, moduleEntry, primeModule, loadPrimeAgent = importPrimeAgent } = {}) {
    ...
    this.sessionPath = sessionPath ? canonicalFile(sessionPath) : null;
  ...
  async start(options = {}) {
    if (this.started && this.connection) return this.waitUntilReady();
    if (this.started) throw new Error("Daemon attachment already started");
    const cwd = options && options.cwd ? path.resolve(options.cwd) : null;
    ...
    try {
      ...
      const findTarget = (sessions) => {
        if (!this.sessionPath || !Array.isArray(sessions)) return null;
        return sessions.find((item) =>
          item && item.activeSessionId && item.sessionFile && canonicalFile(item.sessionFile) === this.sessionPath) || null;
      };

      let listed = await client.request({ type: "list", includeClientOwned: true });
      if (!listed || listed.success !== true || !listed.data || !Array.isArray(listed.data.sessions)) {
        throw new Error(listed && listed.error || "Prime Agent daemon session discovery failed");
      }
      let target = findTarget(listed.data.sessions);

      // No resident match: CREATE a resident worker (not client_owned RPC).
      // client_owned workers are cleaned up ~30s after Desktop disconnects.
      // Never create when the requested session file is missing (deleted/invalid).
      if (!target) {
        ...
        if (this.sessionPath) createPayload.sessionPath = this.sessionPath;
        if (cwd) createPayload.config = { cwd };
        const created = await client.request(createPayload);
        if (!created || created.success !== true || !created.data) {
          // If daemon is missing/unsupported for create, fall back to RPC path.
          const err = new NoResidentSessionError((created && created.error) || "Could not create resident daemon session");
          throw err;
        }
        target = created.data;
        if (target && target.session && !target.activeSessionId) target = target.session;
        if (!target || !target.activeSessionId) {
          listed = await client.request({ type: "list", includeClientOwned: true });
          target = findTarget(listed && listed.data && listed.data.sessions) || target;
        }
      }
      ...
      if (target.sessionFile) {
        const reportedPath = canonicalFile(target.sessionFile);
        if (reportedPath) this.sessionPath = this.sessionPath || reportedPath;
      }

      const connection = await prime.DaemonAgentConnection.attach(client, target.activeSessionId, {
        closeClientOnDispose: true,
        // Never take ownership: dispose must DETACH, never stop the worker.
        sendClientEnv: false,
      ...
      this._seedChildren(this.snapshot && this.snapshot.children);
      const reported = canonicalFile(this.snapshot && this.snapshot.state && this.snapshot.state.sessionFile);
      if (this.sessionPath && reported && reported !== this.sessionPath) {
        throw new Error("Daemon attached a different session than requested");
      }
      if (reported) this.sessionPath = reported;
      this.started = true;
  ...
  async waitUntilReady() {
    ...
    const data = rpcState(state, this.streamingMessage || (this.snapshot && this.snapshot.streamingMessage));
    if (this.streamingMessage || (this.snapshot && this.snapshot.streamingMessage)) data.isStreaming = true;
    data.sessionFile = this.sessionPath || data.sessionFile;
    return response("ready", data);
  }

  _emitRpcEvent(event) {
    // Sub-agent spawn/progress must reach the UI immediately (Agents panel).
    // Buffering these until chat:send completes made children appear ~30–90s late.
  ...
  _observeSessionEvent(event) {
    ...
    if (event.type === "rlm_child_update" && event.child) {
      this._upsertChild(event.child);
      this._emitRpcEvent({ type: "rlm_child_update", child: event.child, children: this._childrenList() });
      return;
    }
    this._emitRpcEvent(event);
  }

  _seedChildren(children) {
    this.childrenById = new Map();
    if (!Array.isArray(children)) return;
    for (const child of children) this._upsertChild(child);
  }
  ...
  _upsertChild(child) {
    ...
    if (!id) return;
    const prev = this.childrenById.get(id) || {};
    this.childrenById.set(id, { ...prev, ...child, id });
  }
  ...
  _replaceSnapshot(snapshot) {
    ...
    if (Array.isArray(snapshot.children)) this._seedChildren(snapshot.children);
  ...
  async command(command) {
    ...
    try {
      switch (type) {
        ...
        case "get_state": {
          const data = await this._currentState();
          // Test-only: allow UI smoke to exercise crash recovery on daemon path.
          if (command.testCrash === true && process.env.PRIME_DESKTOP_TEST_MODE === "1") {
            setTimeout(() => this.emit("exit", { code: 23, error: "Synthetic daemon crash" }), 50);
          }
        ...
        case "list_agents":
        case "get_active_subagents": {
          // Prefer live map (rlm_child_update). Fall back to snapshot seed.
          let agents = this._childrenList();
          if (!agents.length && this.connection && typeof this.connection.getInitialSnapshot === "function") {
            try {
              const snap = await this.connection.getInitialSnapshot();
              if (snap && Array.isArray(snap.children)) {
                this._seedChildren(snap.children);
                agents = this._childrenList();
              }
            } catch {}
          }
          if (!agents.length && this.snapshot && Array.isArray(this.snapshot.children)) agents = this.snapshot.children;
          return response(type, { agents, children: agents });
        }
