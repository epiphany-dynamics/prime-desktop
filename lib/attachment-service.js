...
class AttachmentService {
  ...
  rebindDraftWorkspace(draftId, getWorkspace) {
    if (typeof getWorkspace !== "function") throw new TypeError("A workspace provider is required");
    const draft = this._getDraft(draftId);
    if (draft.sealed || draft.pendingMutations > 0) throw new AttachmentError("ATTACHMENTS_PENDING", "Wait for attachments to finish before restarting");
    const workspace = getWorkspace() || { selected: false, generation: 0 };
    let workspaceRoot = null;
    if (workspace.selected) {
      try { workspaceRoot = fs.realpathSync(workspace.cwd); }
      catch { workspaceRoot = path.resolve(workspace.cwd); }
    }
    this.getWorkspace = getWorkspace;
    draft.workspaceId = workspace.selected ? workspace.workspaceId : null;
    draft.workspaceGeneration = workspace.generation || 0;
