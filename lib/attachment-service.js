...
class AttachmentService {
  ...
  describeDraft(draftId) {
    ...
    return { id: draft.id, workspaceGeneration: draft.workspaceGeneration, items: [...draft.items.values()].map((item) => this._publicItem(item)) };
  }
  ...
  async ingestClipboardImage({ draftId, bytes, name = "Pasted image" }) {
    ...
    return this._ingestImageBuffer(draftId, buffer, safeFilename(name, "Pasted image"));
  }

  async _ingestImageBuffer(draftId, buffer, name, canonicalPath = null, external = false) {
    ...
    // Reject late results after a session/project rotation.
    this._getDraft(draftId);
  ...
  async ingestPaths({ draftId, paths, source = "drop" }) {
    ...
    for (const selectedPath of paths) {
      try { results.push(await this._ingestPath(draftId, selectedPath, source)); }
  ...
  async _ingestPath(draftId, selectedPath, source) {
    const draft = this._getDraft(draftId);
    ...
    if (imageMime) {
      ...
      return this._ingestImageBuffer(draftId, imageBuffer, path.basename(real), real, !inWorkspace);
    }
    this._getDraft(draftId);
    const files = [...draft.items.values()].filter((item) => item.kind === "file");
    if (files.length >= MAX_FILE_REFS) throw new AttachmentError("FILE_COUNT", `Attach up to ${MAX_FILE_REFS} files per message`);
  ...
  ingestReference({ draftId, kind, name, text, dedupeKey }) {
    ...
    if (!["session", "folder"].includes(kind) || typeof text !== "string" || !text || text.length > 100_000) {
      throw new AttachmentError("INVALID_REFERENCE", "That reference could not be attached");
    }
  ...
  remove({ draftId, attachmentId }) {
    ...
    if (!item) return this.describeDraft(draftId);
    ...
    return this.describeDraft(draftId);
  }
  ...
  async sendDraft(payload, sender) {
    ...
    let response;
    try { response = await sender(serialized.command); }
    catch (error) { return { accepted: false, error: error.message || "The message was rejected", serialized }; }
    if (!response || !response.success) return { accepted: false, error: response && response.error || "The message was rejected", response, serialized };
