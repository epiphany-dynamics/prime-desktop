...
class RpcManager extends EventEmitter {
  ...
  async _disposeClient(client, reason = "restart", options = {}) {
    if (!client) return;
    if (client.disposePromise) return client.disposePromise;
    // Default: kill the child. Quit/detach leaves the worker alive so Desktop can
    // reattach via the daemon (or the orphaned process keeps running).
    const killProcess = options.killProcess !== false && reason !== "detach" && reason !== "app-quit";
    client.disposePromise = (async () => {
      client.disposed = true;
      client.intentionalStop = true;
      client.alive = false;
      this._rejectPending(client, killProcess ? `Agent process stopped for ${reason}` : `Desktop detached (${reason})`);
      if (!client.proc || client.exitObserved) return;
      if (!killProcess) {
        // Detach only: do NOT SIGTERM and do NOT close stdin (EOF can exit agents).
        // Drop listeners and unref so Electron can quit while the worker keeps running
        // under the daemon / as an orphaned detached process.
        try {
          if (client.proc.stdout) {
            client.proc.stdout.removeAllListeners();
            if (typeof client.proc.stdout.pause === "function") client.proc.stdout.pause();
            if (typeof client.proc.stdout.unref === "function") client.proc.stdout.unref();
          }
          if (client.proc.stderr) {
            client.proc.stderr.removeAllListeners();
            if (typeof client.proc.stderr.pause === "function") client.proc.stderr.pause();
            if (typeof client.proc.stderr.unref === "function") client.proc.stderr.unref();
          }
          if (client.proc.stdin) {
            client.proc.stdin.removeAllListeners?.();
            // Leave stdin open — closing it is a common "please exit" signal.
            if (typeof client.proc.stdin.unref === "function") client.proc.stdin.unref();
          }
          client.proc.removeAllListeners?.();
          if (typeof client.proc.unref === "function") client.proc.unref();
        } catch {}
        // Drop handle so we cannot accidentally kill it later.
        client.proc = null;
        return;
      }
  ...
  async stop(reason = "shutdown", options = {}) {
    ...
    if (!client) return;
    await this._disposeClient(client, reason, options);
  ...
  async start(input) {
    ...
    let proc;
    try {
      // Detached so agent workers survive Desktop UI quit / crash.
      // Desktop is a client; the worker must outlive the shell.
      proc = this.spawnProcess(invocation.command, args, {
        cwd: targetCwd,
        env,
        stdio: ["pipe", "pipe", "pipe"],
        detached: true,
        // Windows: hide console window if any
        windowsHide: true,
      });
      // Allow Electron main to exit without waiting on these children.
  ...
  _handleMessage(client, message) {
    ...
    if (message && message.type === "response") {
      ...
      // Late/unknown responses belong to expired or stale requests. They are
      // protocol bookkeeping, never renderer-facing agent events.
      return;
    }
