...
function mergeAgentLists(...lists) {
  ...
  for (const list of lists) {
    for (const agent of list || []) {
      ...
      const aRun = !!agent.running;
      const pRun = !!prev.running;
      const aLive = agent.source === "live-snapshot" || agent.source === "live-client" || agent.source === "merged";
      const pLive = prev.source === "live-snapshot" || prev.source === "live-client" || prev.source === "merged";
      // Never let a disk "deleted" row clobber a live running child.
      let status = agent.status || prev.status;
      if (aRun) status = agent.status || "running";
      else if (pRun) status = prev.status || "running";
      else if (aLive && !pLive) status = agent.status || prev.status;
      else if (pLive && !aLive) status = prev.status || agent.status;
      byKey.set(key, {
        ...prev,
        ...agent,
        ...
        running: aRun || pRun,
        status,
        source: aLive || pLive ? "merged" : (agent.source || prev.source),
        updatedAt: agent.updatedAt || prev.updatedAt,
      });
    }
  }
