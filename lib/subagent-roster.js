...
function listSubagentsForParent(primeDir, { parentSessionPath = null, parentSessionId = null } = {}) {
  ...
  const considerRoster = (rosterPath) => {
    ...
    for (const row of parseRosterFile(real)) {
      if (parentReal && row.parentSessionFile) {
        if (safeRealpathSync(row.parentSessionFile) !== parentReal) continue;
      } else if (parentSessionId && row.parentSessionId && row.parentSessionId !== parentSessionId) {
        continue;
      }
  ...
  // Fast path: Prime stores the roster at session-artifacts/<parentSessionId>/rlm-subagents.jsonl
  if (parentSessionId) {
    considerRoster(path.join(artifactsRoot, parentSessionId, "rlm-subagents.jsonl"));
  }

  // If we already have rows for this parent, do NOT scan hundreds of artifact dirs.
  if (rows.length) {
    // fall through to dedupe
  } else if (parentReal) {
    // Slow fallback only when id is unknown: scan a small recent window.
    try {
      ...
      for (const entry of entries) {
        if (parentSessionId && entry.name === parentSessionId) continue;
