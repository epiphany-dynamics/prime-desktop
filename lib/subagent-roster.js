...
function listSubagentsForParent(primeDir, { parentSessionPath = null, parentSessionId = null } = {}) {
  ...
  // Direct folder named by parent session id (Prime layout).
  if (parentSessionId) {
    considerRoster(path.join(artifactsRoot, parentSessionId, "rlm-subagents.jsonl"));
  }

  // Scan artifact roots for parentSessionFile match (bounded).
  try {
    const entries = fs.readdirSync(artifactsRoot, { withFileTypes: true }).slice(0, 400);
    for (const entry of entries) {
      if (!entry.isDirectory()) continue;
      if (parentSessionId && entry.name === parentSessionId) continue; // already
