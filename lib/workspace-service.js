...
const FIXED_IGNORES = new Set([
  ...
  ".ssh", ".gnupg", ".aws", ".azure", ".kube", ".secrets", ".config", ".docker",
]);
const PRIVATE_NAMES = new Set([
  ...
  "application_default_credentials.json", "id_rsa", "id_ed25519", "service-account.json",
]);
...
function isSensitivePath(candidate) {
  ...
  const privateDirectories = new Set([".ssh", ".gnupg", ".aws", ".azure", ".kube", ".secrets", ".git", ".config", ".docker"]);
  ...
  if (/(?:^|[-_.])(?:service[-_]?account|client[-_]?secret)(?:[-_.].*)?\.json$/i.test(name)) return true;
  if (/(?:^|[-_.])firebase(?:[-_.].*)?(?:admin|credential|key|secret)(?:[-_.].*)?\.json$/i.test(name)) return true;
  return false;
}
...
class WorkspaceService {
  ...
  _startWatcher() {
    ...
    if (watcher && typeof watcher.on === "function") {
      watcher.on("error", () => {
        ...
        if (!this.current || this.current.id !== snapshot.id || this.generation !== snapshot.generation) return;
        this.directoryCache.clear();
        this.onInvalidated({ workspaceId: snapshot.id, generation: snapshot.generation, reason: "watcher-error", degraded: true });
      });
    }
  }
  ...
  async _assertReasonableRoot(canonical, options = {}) {
    ...
    const isDirectFilesystemChild = path.dirname(canonical) === filesystemRoot;
    const isVolumeRoot = path.dirname(canonical) === path.join(filesystemRoot, "Volumes");
    const normalized = canonical.split(path.sep).join("/");
    const systemRoots = new Set(["/private/var", "/private/etc", "/usr/local", "/usr/share"]);
    const macSystemVolumes = path.join(filesystemRoot, "System", "Volumes");
    const isMacVolumeContainer = process.platform === "darwin" && canonical === macSystemVolumes;
    const isMacSystemVolumeRoot = process.platform === "darwin" && path.dirname(canonical) === macSystemVolumes;
    let isMountedFilesystemRoot = false;
    if (canonical !== filesystemRoot) {
      try {
        const [candidateStat, parentStat] = await Promise.all([fsp.stat(canonical), fsp.stat(path.dirname(canonical))]);
        isMountedFilesystemRoot = candidateStat.dev !== parentStat.dev;
      } catch {}
    }
    if (canonical === filesystemRoot || isHomeAncestor || isDirectFilesystemChild || isVolumeRoot || isMacVolumeContainer || isMacSystemVolumeRoot || isMountedFilesystemRoot || systemRoots.has(normalized)) {
  ...
  _fixedIgnore(name) {
    ...
    if (/(?:^|[-_.])(?:service[-_]?account|client[-_]?secret)(?:[-_.].*)?\.json$/i.test(normalized)) return true;
    if (/(?:^|[-_.])firebase(?:[-_.].*)?(?:admin|credential|key|secret)(?:[-_.].*)?\.json$/i.test(normalized)) return true;
    return false;
  }
...
const SESSION_WORKSPACE_WARNING = "This session opened, but its saved project is unavailable in Files. Choose a project to restore file browsing.";

async function activateWorkspaceForClient(service, projectPath, options = {}) {
  try {
    const inspected = options.inspected || await service.inspectPath(projectPath);
    const workspace = await service.activatePath(projectPath, {
      inspected,
      recordRecent: options.recordRecent !== false,
    });
    return { workspace, warning: null };
  } catch (error) {
    if (!options.degradeOnFailure) throw error;
    return {
      workspace: service.clear("unavailable-session-project"),
      warning: SESSION_WORKSPACE_WARNING,
    };
  }
}
